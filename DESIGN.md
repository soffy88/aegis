...
**自托管站点/应用的部署与托管**（`api/routers/websites.py`）admit 为受管能力域，**当且仅当**部署产物接入 MAPE-K：入库追踪、健康探活回灌监控、异常经告警管道可达自愈、生命周期操作留审计。未闭环的建站（build-and-forget）仍是 scope creep / 负债，MUST NOT 计为资产。此处闭环红线不放松，反而是建站域的准入执法条件——裸起容器可替代（"能动"判断不变），差异化在于这里的站点是**受管**的。详见 ADR-004。
