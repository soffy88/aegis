...
class TestInstallFlow:
    def test_install_via_dispatcher(
        ...
        with (
            ...
            mock.patch("aegis.server.persistence.event_trail.save_decision_trail", new_callable=mock.AsyncMock),
    ...
    def test_install_without_install_dir_returns_422(self) -> None:
        ...
        with _make_client(ec) as client:
            r = client.post("/api/v1/apps/install", json={"app_name": "nginx"})
    ...
    def test_install_with_empty_install_dir_returns_422(self) -> None:
        ...
        ec = _endpoint_conn()
        with _make_client(ec) as client:
            r = client.post("/api/v1/apps/install", json={"app_name": "nginx", "install_dir": ""})
        assert r.status_code == 422
